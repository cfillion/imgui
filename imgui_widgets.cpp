...
void ImGui::BulletTextV(const char* fmt, va_list args)
{
    ...
    RenderBullet(window->DrawList, bb.Min + ImVec2(style.FramePadding.x + g.FontSize * 0.5f, g.LineHeight * 0.5f), text_col);
...
bool ImGui::CloseButton(ImGuiID id, ImVec2 pos)
{
    ...
    pos += ImVec2(0.0f, (g.LineHeight - size) * 0.5f);
...
void ImGui::ProgressBar(float fraction, const ImVec2& size_arg, const char* overlay)
{
    ...
    ImVec2 pos = window->DC.CursorPos;
    ImVec2 size = CalcItemSize(size_arg, CalcItemWidth(), g.LineHeight + style.FramePadding.y * 2.0f);
...
void ImGui::Bullet()
{
    ...
    ImGuiContext& g = *GImGui;
    const ImGuiStyle& style = g.Style;
    const float line_height = ImMax(ImMin(window->DC.CurrLineSize.y, g.LineHeight + style.FramePadding.y * 2), g.LineHeight);
...
void ImGui::NewLine()
{
    ...
    if (window->DC.CurrLineSize.y > 0.0f)     // In the event that we are on a line with items that is smaller that FontSize high, we will preserve its height.
        ...
        ItemSize(ImVec2(0.0f, g.LineHeight));
...
void ImGui::AlignTextToFramePadding()
{
    ...
    ImGuiContext& g = *GImGui;
    window->DC.CurrLineSize.y = ImMax(window->DC.CurrLineSize.y, g.LineHeight + g.Style.FramePadding.y * 2);
...
static float CalcMaxPopupHeightFromItemCount(int items_count)
{
    ...
    if (items_count <= 0)
        return FLT_MAX;
    return (g.LineHeight + g.Style.ItemSpacing.y) * items_count - g.Style.ItemSpacing.y + (g.Style.WindowPadding.y * 2);
}
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
    ...
    if (!(flags & ImGuiComboFlags_NoArrowButton))
    {
        ...
        {
            const float extra = style.FramePadding.y + (g.LineHeight - g.FontSize) * 0.5f;
...
static ImVec2 InputTextCalcTextSize(ImGuiContext* ctx, const char* text_begin, const char* text_end, const char** remaining, ImVec2* out_offset, bool stop_on_new_line)
{
    ...
    const float line_height = g.LineHeight;
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    const ImGuiID id = window->GetID(label);
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const ImVec2 frame_size = CalcItemSize(size_arg, CalcItemWidth(), (is_multiline ? g.LineHeight * 8.0f : label_size.y) + style.FramePadding.y * 2.0f); // Arbitrary default of 8 lines high for multi-line
    ...
    if (g.ActiveId == id)
    {
        ...
        const float mouse_y = (is_multiline ? (io.MousePos.y - draw_window->DC.CursorPos.y) : (g.LineHeight * 0.5f));
    ...
    if (g.ActiveId == id && !g.ActiveIdIsJustActivated && !clear_active_id)
    {
        ...
        const int row_count_per_page = ImMax((int)((inner_size.y - style.FramePadding.y) / g.LineHeight), 1);
        ...
        else if (IsKeyPressed(ImGuiKey_UpArrow) && is_multiline)     { if (io.KeyCtrl) SetScrollY(draw_window, ImMax(draw_window->Scroll.y - g.LineHeight, 0.0f)); else state->OnKeyPressed((is_startend_key_down ? STB_TEXTEDIT_K_TEXTSTART : STB_TEXTEDIT_K_UP) | k_mask); }
        else if (IsKeyPressed(ImGuiKey_DownArrow) && is_multiline)   { if (io.KeyCtrl) SetScrollY(draw_window, ImMin(draw_window->Scroll.y + g.LineHeight, GetScrollMaxY())); else state->OnKeyPressed((is_startend_key_down ? STB_TEXTEDIT_K_TEXTEND : STB_TEXTEDIT_K_DOWN) | k_mask); }
        else if (IsKeyPressed(ImGuiKey_PageUp) && is_multiline)      { state->OnKeyPressed(STB_TEXTEDIT_K_PGUP | k_mask); scroll_y -= row_count_per_page * g.LineHeight; }
        else if (IsKeyPressed(ImGuiKey_PageDown) && is_multiline)    { state->OnKeyPressed(STB_TEXTEDIT_K_PGDOWN | k_mask); scroll_y += row_count_per_page * g.LineHeight; }
    ...
    if (render_cursor || render_selection)
    {
...

        {
            ...
            cursor_offset.y = cursor_line_no * g.LineHeight;
            if (selmin_line_no >= 0)
            {
                select_start_offset.x = InputTextCalcTextSize(&g, ImStrbol(selmin_ptr, text_begin), selmin_ptr).x;
                select_start_offset.y = selmin_line_no * g.LineHeight;
            }

            // Store text height (note that we haven't calculated text width at all, see GitHub issues #383, #1224)
            if (is_multiline)
                text_size = ImVec2(inner_size.x, line_count * g.LineHeight);
        }
        ...
        if (render_cursor && state->CursorFollow)
        {
            ...
            if (is_multiline)
            {
                ...
                if (cursor_offset.y - g.LineHeight < scroll_y)
                    scroll_y = ImMax(0.0f, cursor_offset.y - g.LineHeight);
        ...
        if (render_selection)
        {
            ...
            {
                if (rect_pos.y > clip_rect.w + g.LineHeight)
                    break;
                ...
                else
                {
                    ...
                    ImRect rect(rect_pos + ImVec2(0.0f, bg_offy_up - g.LineHeight), rect_pos + ImVec2(rect_size.x, bg_offy_dn));
                    rect.ClipWith(clip_rect);
                    if (rect.Overlaps(clip_rect))
                        draw_window->DrawList->AddRectFilled(rect.Min, rect.Max, bg_color);
                    rect_pos.x = draw_pos.x - draw_scroll.x;
                }
                rect_pos.y += g.LineHeight;
            }
        }
        ...
        if (render_cursor)
        {
            ...
            ImRect cursor_screen_rect(cursor_screen_pos.x, cursor_screen_pos.y - g.LineHeight + 0.5f, cursor_screen_pos.x + 1.0f, cursor_screen_pos.y - 1.5f);
            ...
            if (!is_readonly && g.ActiveId == id)
            {
                ...
                ime_data->InputPos = ImVec2(cursor_screen_pos.x - 1.0f, cursor_screen_pos.y - g.LineHeight);
                ime_data->InputLineHeight = g.LineHeight;
    ...
    else
    {
        ...
        if (is_multiline)
            text_size = ImVec2(inner_size.x, InputTextCalcTextLenAndLineCount(buf_display, &buf_display_end) * g.LineHeight); // We don't need width
...
void ImGui::ColorTooltip(const char* text, const float* col, ImGuiColorEditFlags flags)
{
    ...
    ImVec2 sz(g.LineHeight * 3 + g.Style.FramePadding.y * 2, g.LineHeight * 3 + g.Style.FramePadding.y * 2);
...
bool ImGui::TreeNodeBehavior(ImGuiID id, ImGuiTreeNodeFlags flags, const char* label, const char* label_end)
{
    ...
    const float frame_height = ImMax(ImMin(window->DC.CurrLineSize.y, g.LineHeight + style.FramePadding.y * 2), label_size.y + padding.y * 2);
    ...
    // Render
    {
        ...
        if (display_frame)
        {
            ...
            if (flags & ImGuiTreeNodeFlags_Bullet)
                RenderBullet(window->DrawList, ImVec2(text_pos.x - text_offset_x * 0.60f, text_pos.y + g.LineHeight * 0.5f), text_col);
            else if (!is_leaf)
                RenderArrow(window->DrawList, ImVec2(text_pos.x - text_offset_x + padding.x, text_pos.y + (g.LineHeight - g.FontSize) * 0.5f), text_col, is_open ? ((flags & ImGuiTreeNodeFlags_UpsideDownArrow) ? ImGuiDir_Up : ImGuiDir_Down) : ImGuiDir_Right, 1.0f);
        ...
        else
        {
            ...
            if (span_all_columns && !span_all_columns_label)
                TablePopBackgroundChannel();
            if (flags & ImGuiTreeNodeFlags_Bullet)
                RenderBullet(window->DrawList, ImVec2(text_pos.x - text_offset_x * 0.5f, text_pos.y + g.LineHeight * 0.5f), text_col);
            else if (!is_leaf)
                RenderArrow(window->DrawList, ImVec2(text_pos.x - text_offset_x + padding.x, text_pos.y + ((g.LineHeight - g.FontSize) * 0.5f) + (g.FontSize * 0.15f)), text_col, is_open ? ((flags & ImGuiTreeNodeFlags_UpsideDownArrow) ? ImGuiDir_Up : ImGuiDir_Down) : ImGuiDir_Right, 0.70f);
            if (g.LogEnabled)
                LogSetNextTextDecoration(">", NULL);
        }

        if (draw_tree_lines)
            TreeNodeDrawLineToChildNode(ImVec2(text_pos.x - text_offset_x + padding.x, text_pos.y + g.LineHeight * 0.5f));
...
bool ImGui::BeginMenuEx(const char* label, const char* icon, bool enabled)
{
    ...
    else
    {
        ...
        float extra_y = (g.LineHeight - g.FontSize) * 0.5f;
        ImVec2 text_pos(window->DC.CursorPos.x + offsets->OffsetLabel, window->DC.CursorPos.y + window->DC.CurrLineTextBaseOffset);
...
bool ImGui::MenuItemEx(const char* label, const char* icon, const char* shortcut, bool selected, bool enabled)
{
    ...
    else
    {
        ...
        if (g.LastItemData.StatusFlags & ImGuiItemStatusFlags_Visible)
        {
            ...
            if (selected)
            {
                const float extra_y = (g.LineHeight - g.FontSize) * 0.5f;
...
bool    ImGui::BeginTabBar(const char* str_id, ImGuiTabBarFlags flags)
{
    ...
    ImRect tab_bar_bb = ImRect(window->DC.CursorPos.x, window->DC.CursorPos.y, window->WorkRect.Max.x, window->DC.CursorPos.y + g.LineHeight + g.Style.FramePadding.y * 2);
    tab_bar->ID = id;
...
static ImGuiTabItem* ImGui::TabBarScrollingButtons(ImGuiTabBar* tab_bar)
{
    ...
    const ImVec2 arrow_button_size(g.FontSize - 2.0f, g.LineHeight + g.Style.FramePadding.y * 2.0f);
...
static ImGuiTabItem* ImGui::TabBarTabListPopupButton(ImGuiTabBar* tab_bar)
{
    ...
    const float tab_list_popup_button_width = g.LineHeight + g.Style.FramePadding.y;
    const ImVec2 backup_cursor_pos = window->DC.CursorPos;
